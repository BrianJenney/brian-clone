...
export interface LookupWritingArgs {
	query: string;
...
export interface LookupWritingResponse {
...
const RouteSchema = z.object({
	...
	reasoning: z
		.string()
		.describe('One short sentence explaining why those sources were chosen.'),
});
...
async function fetchSource(
	...
	try {
		...
		return raw.map((r) =>
			mapPayload(source, r.id, r.score, (r.payload ?? {}) as Record<string, unknown>),
		);
	} catch (err) {
