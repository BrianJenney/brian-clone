...
export const analyzeChannelTool = tool(
	async (args: { maxVideos?: number }) => {
		try {
			const result = await analyzeChannel(BRIAN_CHANNEL_ID, args.maxVideos || 10);
			return result.stats;
		} catch (error) {
	...
	},
	{
		...
		schema: z.object({
			maxVideos: z.number().optional().describe('Number of recent videos to analyze (default: 10)'),
		}),
	},
);
