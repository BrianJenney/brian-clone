...
import { getRoundsRoles } from '@/libs/mcp/getRoundsRoles';
...
const tracedGetRoundsRoles = traceable(getRoundsRoles, {
	name: 'get_rounds_roles',
	run_type: 'tool',
});
...
function buildServer(): McpServer {
	...
	server.registerTool(
		'get_rounds_roles',
		{
			title: 'Rounds.so recruiter roles catalog',
			description:
				'Returns a page of open roles from Rounds.so (a recruiting platform). Requires ROUNDS_EMAIL and ROUNDS_PASSWORD to be configured. Use cursor to page through results (25 per page).',
			inputSchema: {
				cursor: z
					.number()
					.int()
					.nonnegative()
					.optional()
					.describe('Pagination cursor. Defaults to 0 (first page).'),
			},
		},
		async ({ cursor }) => {
			const payload = await tracedGetRoundsRoles(cursor ?? 0);
			return {
				content: [
					{
						type: 'text' as const,
	...
	server.registerTool(
		...
		async ({ channel, maxVideos }) => {
			const data = await tracedGetYouTubeChannel(
				channel,
				maxVideos ?? 12,
			);
