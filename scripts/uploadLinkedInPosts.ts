...
/**
 * Normalize a CSV createdAt to RFC3339 so Qdrant's datetime index can
 * range-filter it. Keeps the raw value (with a warning) if the shape is
 * unrecognized, rather than dropping data.
 */
function normalizeCreatedAt(raw?: string): string | undefined {
  if (!raw) return undefined;
  const normalized = toRFC3339(raw);
  if (normalized) return normalized;
  console.warn(`Unrecognized createdAt format, storing as-is: ${raw}`);
  return raw;
}
...
function parsePost(
  ...
  return {
    ...
    metadata: {
      ...
      numImpressions: numImpressions ? parseInt(numImpressions) : undefined,
